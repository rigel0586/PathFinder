...
newHoughMap3Dint::newHoughMap3Dint(vector<basicHit> initialHits, FinderParameter FinderParameter)
{
    Map_Maximum3D = 0;
    Map_Maximum3D1 = 0;
    Map_Maximum3D2 = 0;
    ...
    newHoughMap3Dint::calculateDRangeXY();

    trackfound = newHoughMap3Dint::fill();
}

bool newHoughMap3Dint::fill()
{
    //if xy projection
    /*
    ...
    */
    ...
    //if there are no entries in the map track finding cannot be done
    ...
    if(Parameter.getSaveRootFile() == true)
    {
        ...
        FileDoesNotExists1 = Writer1.writeRootFile(&HoughMapInt1,
                                                   &HoughMapRadius_copy, "XYCircularHoughMap_test1.root");
        if(!FileDoesNotExists1)
        {
            Writer1.writeRootFile(&HoughMapInt1, &HoughMapRadius_copy, "XYCircularHoughMap_test2.root");
        }
    }

    //cout<<"newHoughMap3Dint: Number of hits on track: "<<nHitsOnBestTrack<<" hit on track"<<endl;
...
int newHoughMap3Dint::MeanDistanceCalculator(HoughMaximum3D* currentMapMax)
{
    ...
    for(unsigned int iHit = 0; iHit < initialBasicHits.size(); iHit++)
    {
        ...
        DistanceHitTrackCalculator* distanceCalculator = new DistanceHitTrackCalculator(&initialBasicHits.at(iHit),
                                                                                        trackPhi,
                                                                                        trackDZero,
                                                                                        trackOmega);
...
double newHoughMap3Dint::findRadius(HoughMaximum2D* currentMapMax)
{
    ...
    HoughMapRadius.clear();
    calculatedRadii.clear();
    return bestR;
}//double newHoughMap3Dint::findRadius()


HoughMaximum3D* newHoughMap3Dint::findMaximum()
{
    return Map_Maximum3D;
}//HoughMaximum3D* newHoughMap3Dint::findMaximum()

void newHoughMap3Dint::findMaximum2D()
{
...
void newHoughMap3Dint::calculateDRangeXY()
{
...
void newHoughMap3Dint::print() const
{
